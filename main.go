...
func (c *flashcards) exportCards(expPath string) {
	...
	msg = fmt.Sprintf("%d cards have been saved.", len(c.TermDef))
	c.writeLogEntry(msg, true)
}

func (c *flashcards) getUserInput() {
	...
	for i := 0; i < num; i++ {
		...
		if userDef == def {
			...
			c.err[term] += 1
			if c.err[term] > c.highestErrorCount {
				c.highestErrorCount = c.err[term]
			}
...
func (c *flashcards) printHardestCards() {
	if len(c.err) == 0 {
		...
		msg := fmt.Sprintf("The hardest card %s \"%s\". You have %d errors answering it.",
			func() string {
				if len(cardsMap[c.highestErrorCount]) > 1 {
					return "are"
				} else {
					return "is"
				}
			}(),
			strings.Join(cardsMap[c.highestErrorCount], ", "), c.highestErrorCount)
